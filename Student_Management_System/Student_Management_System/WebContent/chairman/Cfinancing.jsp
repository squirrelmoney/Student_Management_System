    ...
    <div class="content-wrap">
        <div class="main">
			...
			<tbody>
             ...
             select * from financing where Acno in(
             select Acno from activities
             where Ano='<%=session.getAttribute("Ano")%>')
             and Access<>'undercheck';
            </sql:query>
             <c:forEach var="re" items="${result.rows}">
            <thead>
			<tr>
            <td><c:out value="${re.Fno}"></c:out></td>
            <td><c:out value="${re.Acno}"></c:out></td>
            <td><c:out value="${re.Mno}"></c:out></td>
            <td><c:out value="${re.Budget}"></c:out></td>
			  <td><c:out value="${re.Access}"></c:out></td>
             <td><a href="updateFinancing.jsp?f=update&Fno=${re.Fno}">修改</a></td>
			 <td><a href="updatefianncing1?f=delete&Acno=${re.Fno}"
